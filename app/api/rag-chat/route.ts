...
export async function POST(req: Request) {
  try {
    ...
    // Extract the filename and user input
    ...
    const ragRequest = {
      query: user_input,
      filename: filename,
      k: 3, // Default to 3 chunks if not specified
    }
    ...
    return NextResponse.json(
      {
        error: "Failed to process the RAG chat request",
        ...
        conversation_history: [
          ...
          {
            role: "assistant",
            content: "Sorry, there was an error processing your document-based query. Please try again.",
          },
        ],
      },
      { status: 500 },
